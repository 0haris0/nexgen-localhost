...
import {
  ...
  Text,
  BlockStack,
  InlineStack, ButtonGroup, Button, Tag,
} from '@shopify/polaris'
import { CheckIcon, DomainNewIcon, PlusIcon, XIcon } from '@shopify/polaris-icons'
...
const SingleProduct = (props) => {
  ...
  function checkImageSize (width, height) {
    ...
    } else if (areaDifference <= 0.10) {
      sizeLevel = 4 // Very close to perfect
    } else if (areaDifference <= 0.20) {
      sizeLevel = 3 // Slightly off
    } else if (areaDifference <= 0.30) {
      sizeLevel = 2 // Noticeable deviation
    } else if (areaDifference <= 0.50) {
    ...
    <Card title={product.title} sectioned>
      ...
      <BlockStack gap="4">
        ...
        <InlineStack gap="4" align={'start'}>
          ...
          <Text
            as="span">{product.category?.fullName || product.category?.name || 'N/A'}</Text>
          <Text as="span" fontWeight="bold">
            Product Type:
          </Text>
        ...
        <InlineStack gap={2}>
          <Text as="span" fontWeight="bold">Recomended tags:</Text>
          <Tag tone={'new'} icon={DomainNewIcon}
               onClick={() => handleTag('Tag 1')}>Tag 1</Tag>
        </InlineStack>

        {/* Informacije o Mediji */}
        {product.featuredImage ?
          <Text as={'span'} fontWeight={'bold'} tone={'critical'}>Image
            size: {product.featuredImage?.width} x {product.featuredImage?.height} {ImageEvaluation({
              width : product.featuredImage?.width,
              height: product.featuredImage?.height,
            })}
            {checkImageSize(product.featuredImage?.width, product.featuredImage?.height)}</Text>
          : null}
